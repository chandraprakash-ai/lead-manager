...
import type { Lead, NicheCategory } from '../types';
...
// Map user columns to DB columns
type DBField = keyof Omit<Lead, 'id' | 'created_at'>;
        ...
        reader.onload = (evt) => {
            try {
                ...
                headers.forEach(h => {
                    ...
                    else if (normalized.includes('address')) autoMap[h] = 'address';
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            const leadsToInsert = fileData.map((row: any[]) => {
                // ... (Transformation logic remains identical)
                const lead: any = {
                    contacted: false,
                    priority: 'High',
                    deal_stage: 'New',
